...
CREATE PROCEDURE QUOCTINHKHAC (@Tenqg NVARCHAR(50)) AS
BEGIN 
    SELECT CAULACBO.MACLB, CAULACBO.TENCLB, SANVD.TENSAN, SANVD.DIACHI, COUNT(CAUTHU.MACT) AS CTKHACNGUOC FROM CAULACBO
    JOIN SANVD ON CAULACBO.MASAN = SANVD.MASAN
    JOIN CAUTHU ON CAULACBO.MACLB = CAUTHU.MACLB 
    WHERE CAUTHU.MAQG <> @Tenqg
    GROUP BY CAULACBO.MACLB, CAULACBO.TENCLB, SANVD.TENSAN, SANVD.DIACHI
    HAVING COUNT(CAUTHU.MACT) > 2
END 
